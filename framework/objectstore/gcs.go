...
func NewGCSObjectStore(ctx context.Context, cfg *Config, logger schemas.Logger) (*GCSObjectStore, error) {
	...
	if cfg.Credentials != nil && cfg.Credentials.GetValue() != "" {
		creds := cfg.Credentials.GetValue()
...
// Ping checks connectivity by attempting to read a non-existent object.
// This only requires storage.objects.get (Storage Object User role), unlike
// Bucket.Attrs() which requires storage.buckets.get.
func (g *GCSObjectStore) Ping(ctx context.Context) error {
	_, err := g.client.Bucket(g.bucket).Object("__bifrost_ping__").Attrs(ctx)
